...
import com.shwm.freshmallpos.util.ColorUtil;
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof DefaultViewHolder) {
            ...
            if (order.getRefund().equals("已退款")) {
                ...
                spanMoney.setSpan(new ForegroundColorSpan(ColorUtil.ColorRed), 0, spanMoney.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
            ((DefaultViewHolder) holder).tvMoney.setText(spanMoney);
            ((DefaultViewHolder) holder).tvTime.setText(order.getTime() + "");
            if (StringUtil.isNoEmpty(order.getRefund())) {
                SpannableString spanRefund = new SpannableString(order.getRefund());
                spanRefund.setSpan(new ForegroundColorSpan(ColorUtil.ColorOrange), 0, spanRefund.length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
                ((DefaultViewHolder) holder).tvRefund.setText(spanRefund);
            }else {
                ((DefaultViewHolder) holder).tvRefund.setText("");
            }
        }
