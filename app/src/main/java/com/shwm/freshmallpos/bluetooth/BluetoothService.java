...
public class BluetoothService {
	...
	private static final UUID MY_UUID = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB"); // change by chongqing
																									// jinou
	// Member fields
	...
	private static BluetoothDevice mBluetoothDivceConnect;// 连接的蓝牙
	private static String weightStr;// 称重数量
	...
	public void printSize(int size) {
		...
		switch (size) {
		case UtilBluetoothValue.Text_Size_large2:
			write(byteCommands[4]);
			break;
		case UtilBluetoothValue.Text_Size_large6:
			write(byteCommands[5]);
			break;
		case UtilBluetoothValue.Text_Size_default:
	...
	private synchronized void setState(int state) {
		...
		if (mHandler != null)
			mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
	}
	...
	/**
	 * Start the service. Specifically start AcceptThread to begin a session in listening (server) mode. Called by the
	 * Activity onResume()
	 */
	...
	public synchronized void connected(BluetoothSocket socket, BluetoothDevice device) {
		...
		if (mHandler != null) {
			Message msg = mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_DEVICE_NAME);
			Bundle bundle = new Bundle();
			bundle.putParcelable(UtilBluetoothValue.DEVICE, device);
			msg.setData(bundle);
			mHandler.sendMessage(msg);
		}
		setState(STATE_CONNECTED);
		mBluetoothDivceConnect = device;
	}
	...
	private void connectionFailed() {
		...
		if (mHandler != null) {
			Message msg = mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_TOAST);
			Bundle bundle = new Bundle();
			bundle.putString(UtilBluetoothValue.TOAST, "无法连接设备");
			msg.setData(bundle);
			mHandler.sendMessage(msg);
		}
		mBluetoothDivceConnect = null;
	...
	private void connectionLost() {
		...
		if (mHandler != null) {
			Message msg = mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_TOAST);
			Bundle bundle = new Bundle();
			bundle.putString(UtilBluetoothValue.TOAST, "设备连接丢失");
			msg.setData(bundle);
			mHandler.sendMessage(msg);
		}
		mBluetoothDivceConnect = null;
	...
	private class ConnectedThread extends Thread {
		...
		public void run() {
			...
			while (true) {
				try {
					...
					// bytes = mmInStream.read(buffer, 32);
					// bytes = mmInStream.read(buffer, 0, 32);
					bytes = mmInStream.read(buffer);
					if (bytes > 0) {
						UL.d(TAG, "bytes>0");
						// Send the obtained bytes to the UI Activity
						String s = new String(buffer, 0, bytes);
						// Log.e(TAG, "读取:" + s + " len:" + s.length());
						if (mHandler != null) {
							mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_READ, bytes, -1, buffer).sendToTarget();
						}
						// 称重
						if (bytes > 6) {
							String data = new String(buffer, 0, bytes);
							List<String> listData = StringUtil.matchBeforeAfter("=", "=", data);
							if (listData != null && listData.size() > 0) {
								weightStr = listData.get(0);
								weightStr = StringUtil.reverse(weightStr);
								UL.d(TAG, "weight=" + weightStr);
							}
						}
					} else {
		...
		public void write(byte[] buffer) {
			try {
				...
				if (mHandler != null)
					mHandler.obtainMessage(UtilBluetoothValue.MESSAGE_WRITE, -1, -1, buffer).sendToTarget();
			} catch (IOException e) {
	...
	public static BluetoothDevice getmBluetoothDivceConnect() {
		return mBluetoothDivceConnect;
	}

	public static double getWeight() {
		double weight = StringUtil.getDouble(weightStr);
		weight = UtilMath.div(weight, 1, 2);
		return weight;
	}
