...
public class SwipeDeleteView extends ViewGroup {
	...
	/**
	 * 仿QQ，侧滑菜单展开时，点击除侧滑菜单之外的区域，关闭侧滑菜单。 增加一个布尔值变量，dispatch函数里，每次down时，为true，move时判断，如果是滑动动作，设为false。
	 ...
	 * @param ev
	 *            事件分发
	 * @return 如果返回值返回的是true：表示消费了此次事件，没有谁能再收到这个事件，事件终止 返回false，则回传给父控件的onTouchEvent处理 直接返回super.xxxx
	 *         表示如果当前时ViewGroup则直接传递给自己的onInterceptTouchEvent， 如果onInterceptTouchEvent返回false或者super.xxx
	 *         则表示继续分发给下一级的dispatchTouchEvent 如果onInterceptTouchEvent返回true，则交给自己的onTouchEvent处理
	 */
	@Override
	...
	public void smoothClose() {
		...
		mCloseAnimator.setInterpolator(new AnticipateInterpolator());// AnticipateInterpolator():开始的时候向后然后向前甩
	...
	/**
	 * 该方法是在销毁View的时候调用 每次ViewDetach的时候，判断一下 ViewCache是不是自己，如果是自己，关闭侧滑菜单，且ViewCache设置为null， 理由：1
	 * 防止内存泄漏(ViewCache是一个静态变量) 2 侧滑删除自己后，这个View被Recycler回收，复用，下一个进入屏幕的View的状态应该是普通状态，而不是展开状态。
	 */
	@Override
	...
	/**
	 * 快速关闭。 用于 点击侧滑菜单上的选项(比如：删除 、置顶等),同时想让它快速关闭。 这个方法在ListView里是必须调用的，
