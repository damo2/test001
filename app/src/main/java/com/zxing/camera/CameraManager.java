...
public final class CameraManager {
	...
	// 在这里修改扫描框的最小、最大的宽、高值
	public static int MIN_FRAME_WIDTH = UtilPx.dp2px(ApplicationMy.getContext(), 160);
	public static int MIN_FRAME_HEIGHT = UtilPx.dp2px(ApplicationMy.getContext(), 120);
	public static int MAX_FRAME_WIDTH = UtilPx.dp2px(ApplicationMy.getContext(), 320);
	public static int MAX_FRAME_HEIGHT = UtilPx.dp2px(ApplicationMy.getContext(), 320);
	...
	public boolean isOffLight() {
		if (context != null && !context.getPackageManager().hasSystemFeature(PackageManager.FEATURE_CAMERA_FLASH)) {
		...
		return Parameters.FLASH_MODE_OFF.equals(parameters.getFlashMode());
	}

	/** 打开闪光灯 */
	public void openOnLight() {
		openCloseLight(true);
	}

	/** 关闭闪光灯 */
	public void closeOffLight() {
		openCloseLight(false);
	}

	// 闪光灯开关
	private void openCloseLight(boolean isOpen) {
		if (camera == null) {
			camera = Camera.open();
		}
		if (parameters == null) {
